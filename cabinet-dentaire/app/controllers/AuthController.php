...
class AuthController extends Controller
{
    ...
    public function authenticate()
    {
        ...
        // Déterminer le rôle valide de l'utilisateur
        $userRole = strtolower(trim($user['role'] ?? ''));
        if (!in_array($userRole, ['patient', 'medecin', 'dentiste', 'secretaire', 'admin'])) {
            $medecinModel = new Medecin();
            if ($medecinModel->findByUserId($user['id'])) {
                $userRole = 'dentiste';
                $this->userModel->update($user['id'], ['role' => $userRole]);
            } else {
                $secretaireModel = new Secretaire();
                if ($secretaireModel->findByUserId($user['id'])) {
                    $userRole = 'secretaire';
                    $this->userModel->update($user['id'], ['role' => $userRole]);
                } else {
                    $patientModel = new Patient();
                    if ($patientModel->findByUserId($user['id'])) {
                        $userRole = 'patient';
                        $this->userModel->update($user['id'], ['role' => $userRole]);
                    }
                }
    ...
    public function logout()
    {
        ...
        // Vider la session côté serveur et client
        $_SESSION = [];
