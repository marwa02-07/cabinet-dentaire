...
class Medecin extends Model
{
    ...
    public function create($data)
    {
        try {
            ...
            if (!$result) {
                ...
                error_log("Erreur lors de l'insertion du médecin: " . json_encode($errorInfo));
    ...
    /**
     * Trouver un médecin par numéro de licence
     */
    public function findByNumeroLicence($numero_licence)
    {
        try {
            $query = "SELECT * FROM dentistes WHERE numero_licence = :numero_licence LIMIT 1";
            $stmt = $this->pdo->prepare($query);
            $stmt->bindParam(':numero_licence', $numero_licence);
            $stmt->execute();
            return $stmt->fetch(PDO::FETCH_ASSOC);
        } catch (PDOException $e) {
            error_log("Erreur PDO findByNumeroLicence: " . $e->getMessage());
            return false;
        }
    }
    ...
    public function getBySpecialite($specialite)
    {
        try {
            ...
            // Mots-clés par type de spécialite
            $keywordsMap = [
                'consultation' => ['consultation', 'chirurgie', 'dentaire', 'généraliste', 'général'],
                'nettoyage' => ['nettoyage', 'hygiene', 'hygiène', 'prophylaxie'],
                'extraction' => ['extraction', 'chirurgie', 'orale', 'chirurgical'],
                'traitement' => ['traitement', 'endodontie', 'restaurateur', 'obturation'],
                'blanchiment' => ['blanchiment', 'esthétique', 'esthetique', 'cosmétique', 'cosmetique'],
                'radio' => ['radio', 'imagerie', 'radiologie', 'scanner']
            ];
    ...
    public function update($id, $data)
    {
        try {
            ...
            // Préparer les valeurs - utiliser les valeurs existantes si non fournies
            $specialite = $data['specialite'] ?? $medecin['specialite'];
            $telephone = $data['telephone'] ?? $medecin['telephone'];
            $numero_licence = $data['numero_licence'] ?? $medecin['numero_licence'];
            $cabinet = $data['cabinet'] ?? $medecin['cabinet'];
            
            // Mise à jour dans la table dentistes
            $query = "UPDATE dentistes SET specialite = :specialite, telephone = :telephone, numero_licence = :numero_licence, cabinet = :cabinet WHERE id = :id";
